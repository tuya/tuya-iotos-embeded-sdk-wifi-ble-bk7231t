...
const char *CFG_GetDeviceName() {
	return "obk_demo_scanNetworks";
}

static int setup_wifi_open_access_point(void)
{
    #define APP_DRONE_DEF_SSID          "OpenBK2731T_WiFi_Demo"
...
static void app_led_timer_handler(void *data)
{
	...
	} else if(openAPAfterDelay == 0 && g_secondsElapsed % 5 == 0) {
		doScan();
	}
}
...
void doScan()
{
	...
	for(i = 0; i < num; i++) {
		bk_printf("[%i/%i] SSID: %s, Channel: %i, Signal %i\r\n",i,num,ar[i].ssid, ar[i].channel, ar[i].rssi);
	}
